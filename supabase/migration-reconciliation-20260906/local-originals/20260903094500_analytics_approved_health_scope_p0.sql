-- P0 analytics correction: align operational assurance with approved formal prices,
-- preserving incident debounce, notifications, and automatic recovery.
...
begin
  ...
  loop
    ...
    into total_count, recent_count, missing_date_count, latest_record_at
    from (
      ...
      where equipment.organization_id = organization_row.id
        ...
        and equipment.review_status = 'approved'
      union all
      ...
      where material.organization_id = organization_row.id
        and material.review_status = 'approved'
    ) price;
    ...
    next_status := case
      ...
      when missing_date_count::numeric / greatest(total_count, 1) > 0.20 then 'warning'
      else 'healthy'
    end;
    next_message := case
      ...
      when missing_date_count::numeric / greatest(total_count, 1) > 0.20 then format('%s 条价格缺少价格日期，超过 20%% 质量阈值', missing_date_count)
    ...
    insert into public.wpi_automation_component_states (
      ...
      organization_row.id, 'runtime:analytics-aggregation', 'runtime', '统计分析正式数据质量', next_status,
      ...
      jsonb_build_object(
        ...
        'missingPriceDateCount', missing_date_count,
        'missingPriceDateRatio', round(missing_date_count::numeric / greatest(total_count, 1) * 100, 1),
