-- CAID also publishes brochures and other statistical bulletins as PDF files.
-- Keep those documents as evidence, but only parse explicitly dated LOKOLE
-- monthly price reports into price leads.
...
where discovery.resource_type = 'pdf'
  ...
  and not (
    lower(discovery.resource_url) ~ '/lokole/.*(janvier|fevrier|mars|avril|mai|juin|juillet|aout|sept(embre)?|octobre|novembre|decembre)[^/]*20[0-9]{2}'
  )
  and exists (
    select 1
...
where not (
    lower(evidence.source_url) ~ '/lokole/.*(janvier|fevrier|mars|avril|mai|juin|juillet|aout|sept(embre)?|octobre|novembre|decembre)[^/]*20[0-9]{2}'
  )
  and exists (
    select 1
    from public.wpi_price_collection_sources as source
