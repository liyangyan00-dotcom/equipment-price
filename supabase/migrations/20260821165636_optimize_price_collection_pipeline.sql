...
create policy wpi_currency_rates_insert on public.wpi_currency_rates for insert to authenticated
  with check (private.wpi_has_permission(organization_id, 'settings.manage'::public.wpi_app_permission));
create policy wpi_currency_rates_update on public.wpi_currency_rates for update to authenticated
  using (private.wpi_has_permission(organization_id, 'settings.manage'::public.wpi_app_permission))
  with check (private.wpi_has_permission(organization_id, 'settings.manage'::public.wpi_app_permission));
create policy wpi_currency_rates_delete on public.wpi_currency_rates for delete to authenticated
  using (private.wpi_has_permission(organization_id, 'settings.manage'::public.wpi_app_permission));

drop policy if exists wpi_collection_sources_write on public.wpi_price_collection_sources;
create policy wpi_collection_sources_insert on public.wpi_price_collection_sources for insert to authenticated
  with check (private.wpi_has_permission(organization_id, 'settings.manage'::public.wpi_app_permission));
create policy wpi_collection_sources_update on public.wpi_price_collection_sources for update to authenticated
  using (private.wpi_has_permission(organization_id, 'settings.manage'::public.wpi_app_permission))
  with check (private.wpi_has_permission(organization_id, 'settings.manage'::public.wpi_app_permission));
create policy wpi_collection_sources_delete on public.wpi_price_collection_sources for delete to authenticated
  using (private.wpi_has_permission(organization_id, 'settings.manage'::public.wpi_app_permission));

drop policy if exists wpi_collection_runs_write on public.wpi_price_collection_runs;
create policy wpi_collection_runs_insert on public.wpi_price_collection_runs for insert to authenticated
  with check (private.wpi_has_permission(organization_id, 'price.write'::public.wpi_app_permission));
create policy wpi_collection_runs_update on public.wpi_price_collection_runs for update to authenticated
  using (private.wpi_has_permission(organization_id, 'price.write'::public.wpi_app_permission))
  with check (private.wpi_has_permission(organization_id, 'price.write'::public.wpi_app_permission));
create policy wpi_collection_runs_delete on public.wpi_price_collection_runs for delete to authenticated
  using (private.wpi_has_permission(organization_id, 'price.write'::public.wpi_app_permission));

drop policy if exists wpi_collection_evidence_write on public.wpi_price_collection_evidence;
create policy wpi_collection_evidence_insert on public.wpi_price_collection_evidence for insert to authenticated
  with check (private.wpi_has_permission(organization_id, 'price.write'::public.wpi_app_permission));
create policy wpi_collection_evidence_update on public.wpi_price_collection_evidence for update to authenticated
  using (private.wpi_has_permission(organization_id, 'price.write'::public.wpi_app_permission))
  with check (private.wpi_has_permission(organization_id, 'price.write'::public.wpi_app_permission));
create policy wpi_collection_evidence_delete on public.wpi_price_collection_evidence for delete to authenticated
  using (private.wpi_has_permission(organization_id, 'price.write'::public.wpi_app_permission));;
