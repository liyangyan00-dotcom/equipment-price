...
create policy wpi_inquiry_letter_versions_read
  on public.wpi_inquiry_letter_versions for select to authenticated
  using (private.wpi_has_permission(organization_id, 'inquiry.read'));
create policy wpi_inquiry_letter_versions_insert
  on public.wpi_inquiry_letter_versions for insert to authenticated
  with check (private.wpi_has_permission(organization_id, 'inquiry.write') and created_by = auth.uid());
