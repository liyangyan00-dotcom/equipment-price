...
begin
  ...
  payload_evidence_checks := coalesce(
    review_payload -> 'evidenceChecks',
    '{}'::jsonb
  );
  ...
  if review_row.id is null then
    ...
    set
      status = 'pending',
      confidence = coalesce(
        (review_payload ->> 'confidence')::numeric,
        review_row.confidence
      ),
      completeness = coalesce(
        (review_payload ->> 'completeness')::numeric,
        review_row.completeness
      ),
      risk_level = coalesce(
        nullif(review_payload ->> 'riskLevel', '')::public.wpi_risk_level,
