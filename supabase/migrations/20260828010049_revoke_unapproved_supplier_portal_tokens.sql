...
begin
  ...
  if p_decision not in ('pending_review', 'approved', 'rejected') then raise exception 'INVALID_SUPPLIER_REVIEW_DECISION'; end if;
  if coalesce(array_length(p_supplier_ids, 1), 0) = 0 then raise exception 'SUPPLIER_IDS_REQUIRED'; end if;
  if v_notes is null or char_length(v_notes) < 5 then raise exception 'SUPPLIER_REVIEW_NOTES_REQUIRED'; end if;
  select membership.organization_id into v_organization_id
  from public.wpi_organization_members membership
  where membership.user_id = v_actor and membership.is_active = true
  order by membership.joined_at limit 1;
  if v_organization_id is null or not private.wpi_has_permission(v_organization_id, 'supplier.review') then
    raise exception 'SUPPLIER_REVIEW_FORBIDDEN';
  end if;
  if (select count(*) from public.wpi_suppliers supplier
      where supplier.organization_id = v_organization_id and supplier.id = any(p_supplier_ids))
  ...
  if p_decision = 'approved' then
    select count(*) into v_missing_legal_names from public.wpi_suppliers supplier
    where supplier.organization_id = v_organization_id and supplier.id = any(p_supplier_ids)
      and nullif(btrim(coalesce(supplier.legal_name, '')), '') is null;
    if v_missing_legal_names > 0 then raise exception 'SUPPLIER_LEGAL_ENTITY_REQUIRED'; end if;
  end if;
  ...
  if p_decision <> 'approved' then
    update public.wpi_inquiry_portal_tokens token
    set status = 'revoked'
    where token.organization_id = v_organization_id
      and token.supplier_id = any(p_supplier_ids)
  ...
  set review_status = p_decision,
      metadata = supplier.metadata || jsonb_build_object(
        'humanReview', jsonb_build_object(
          'status', p_decision, 'reviewedBy', v_actor, 'reviewedAt', now(), 'notes', v_notes,
          'duplicateResolution', case when p_decision = 'approved' then 'consolidated' else 'pending' end
        ),
...
where token.status = 'active'
  and not exists (
    select 1
    from public.wpi_suppliers supplier
    where supplier.id = token.supplier_id
      and supplier.organization_id = token.organization_id
      and supplier.review_status = 'approved'
  );
;
