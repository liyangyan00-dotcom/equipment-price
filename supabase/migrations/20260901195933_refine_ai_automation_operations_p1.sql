...
declare
  ...
  collection_failures_24h integer := 0;
  overall_status text;
begin
  ...
  where task.organization_id = target_organization_id
    and run.run_kind = 'source_attempt'
  ...
  overall_status := case
    ...
    when blocked_workflows > 0 or integration_errors > 0
      or failed_tasks_24h > 0 or collection_failures_24h > 0 then 'degraded'
    else 'healthy'
  end;
  ...
  return jsonb_build_object(
    ...
    'canManage', coalesce(member_role in ('admin', 'manager'), false)
