...
as $$
  ...
  select jsonb_build_object(
    'rows', coalesce((select jsonb_agg(to_jsonb(page_rows) - 'has_evidence' - 'has_fx_issue' - 'has_stale_source' order by created_at desc, id desc) from page_rows), '[]'::jsonb),
...
comment on function public.wpi_search_price_collection_leads_v4(
  text, uuid, text, text, text, text, text, numeric, numeric, numeric, numeric,
  date, date, text, text, text, integer, integer
) is 'P1 price lead search with server-side admission issue filters and consistent issue totals.';
