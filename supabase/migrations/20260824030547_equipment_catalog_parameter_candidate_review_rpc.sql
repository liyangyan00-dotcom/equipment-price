...
begin
  ...
  where id = p_candidate_id
    and equipment_catalog_id = p_catalog_id
  for update;

  if not found then
    raise exception 'PARAMETER_CANDIDATE_NOT_FOUND';
  end if;
  if v_candidate.review_decision <> 'pending' then
    raise exception 'PARAMETER_CANDIDATE_ALREADY_REVIEWED';
  end if;
  ...
  if p_decision = 'accepted' then
    insert into public.wpi_equipment_catalog_parameters (
      ...
      v_candidate.equipment_catalog_id,
      v_candidate.parameter_code,
      v_candidate.parameter_name,
      v_candidate.proposed_value,
      v_candidate.proposed_value,
      'text',
      coalesce(v_candidate.proposed_unit, ''),
      v_candidate.risk_level in ('high', 'critical'),
      ...
      jsonb_build_object(
        ...
        'extractionMethod', 'openai_responses_pdf',
        'humanConfirmed', true,
  ...
  from public.wpi_equipment_catalog_parameter_candidates
  where job_id = v_candidate.job_id
  ...
  set review_status = 'pending_review',
      updated_by = (select auth.uid()),
      updated_at = v_reviewed_at
  where id = v_candidate.equipment_catalog_id
    and organization_id = v_candidate.organization_id;
...
comment on function public.wpi_review_equipment_catalog_parameter_candidate(uuid, uuid, text, text) is
  '原子确认设备 PDF 参数候选；接受时写入正式参数，拒绝时仅保留证据审计记录。'
