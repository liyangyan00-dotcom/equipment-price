...
declare
  ...
  row_item jsonb;
begin
  select * into batch_row from public.wpi_equipment_import_batches
  ...
  update public.wpi_equipment_import_batches set
    ...
    source_metadata = coalesce(source_metadata, '{}'::jsonb) || coalesce(batch_payload -> 'sourceMetadata', '{}'::jsonb)
  where id = target_batch_id;
  delete from public.wpi_import_field_mappings where batch_id = target_batch_id;
  for mapping_item in select value from jsonb_array_elements(coalesce(mapping_payload, '[]'::jsonb)) loop
    insert into public.wpi_import_field_mappings (
      organization_id,batch_id,source_field,system_field,sample_value,confidence,mapping_status,is_required,user_modified
    ) values (
      batch_row.organization_id,target_batch_id,mapping_item ->> 'sourceField',
      nullif(mapping_item ->> 'systemField', ''),nullif(mapping_item ->> 'sampleValue', ''),
  ...
  delete from public.wpi_equipment_import_rows where batch_id = target_batch_id;
  for row_item in select value from jsonb_array_elements(coalesce(row_payload, '[]'::jsonb)) loop
    insert into public.wpi_equipment_import_rows (
      organization_id,batch_id,row_number,raw_data,normalized_data,validation_status,confidence,issues,is_selected
    ) values (
      batch_row.organization_id,target_batch_id,
      greatest(1, coalesce(nullif(row_item ->> 'rowNumber', '')::integer, 1)),
      jsonb_build_object(
        '设备名称',coalesce(row_item ->> 'equipmentName',''),'规格型号',coalesce(row_item ->> 'model',''),
        '品牌',coalesce(row_item ->> 'brand',''),'设备类别',coalesce(row_item ->> 'category',''),
        '原始价格',coalesce(nullif(row_item ->> 'originalPrice','')::numeric,0),
        '币种',coalesce(row_item ->> 'currency',''),'供应商',coalesce(row_item ->> 'supplier',''),
        '报价日期',coalesce(row_item ->> 'quoteDate','')
      ),
      jsonb_build_object(
        'equipment_name',coalesce(row_item ->> 'equipmentName',''),'model',coalesce(row_item ->> 'model',''),
        'brand',coalesce(row_item ->> 'brand',''),'category',coalesce(row_item ->> 'category',''),
        'original_price',coalesce(nullif(row_item ->> 'originalPrice','')::numeric,0),
        'original_currency',coalesce(row_item ->> 'currency',''),
        'supplier_name',coalesce(row_item ->> 'supplier',''),'quote_date',coalesce(row_item ->> 'quoteDate','')
      ),
  ...
  select * into batch_row from private.wpi_refresh_equipment_import_batch(target_batch_id);
  return batch_row;
end;
$$;
revoke all on function public.wpi_save_equipment_import_draft(uuid,jsonb,jsonb,jsonb) from public, anon;
grant execute on function public.wpi_save_equipment_import_draft(uuid,jsonb,jsonb,jsonb) to authenticated;;
