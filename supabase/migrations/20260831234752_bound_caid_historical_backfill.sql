...
metadata=coalesce(discovery.metadata,'{}'::jsonb)||jsonb_build_object('trackedOnly',true,'parseStatus','archived','historicalBackfillAvailable',true,'requiresHumanReview',true),
updated_at=now()
where discovery.resource_type='pdf' and discovery.status in ('queued','failed')
