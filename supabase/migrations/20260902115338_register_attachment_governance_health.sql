...
declare
  ...
  new_jobs text := $jobs$'wpi-price-collection-due',
        ...
        'wpi-attachment-governance'$jobs$;
begin
  ...
  if updated_definition = function_definition then
    raise exception 'Unable to add attachment governance cron to automation health';
  end if;
...
comment on function public.wpi_get_ai_automation_health(uuid) is
  'Returns agent runtime health with configured jobs, including attachment evidence governance.';
