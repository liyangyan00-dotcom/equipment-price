        ...
        {data.map((action, index) => {
          ...
          return (
            <Link
              key={action.title}
              href={action.href}
              className="group flex min-h-[64px] items-center gap-2.5 rounded-[13px] border border-borderSoft bg-white/72 px-3 transition hover:border-primary hover:bg-white hover:shadow-card"
            >
              <span className={`flex size-9 shrink-0 items-center justify-center rounded-[11px] border shadow-sm ${toneClass[action.tone]}`}>
                <Icon className="size-[18px]" aria-hidden="true" />
              </span>
              <span className="min-w-0">
                <span className={`block truncate text-[13px] font-semibold ${textToneClass[action.tone]}`}>{action.title}</span>
                <span className={`mt-1 block truncate text-[11px] ${textToneClass[action.tone]}`}>{action.description}</span>
              </span>
            </Link>
          );
        })}
