...
const featureCards = [
  ...
  { title: "刚果金地材价格库", description: "区域地材价格数据与趋势分析" },
  { title: "AI 报价识别", description: "智能识别报价单，辅助提取结构化价格" },
  { title: "询价比价管理", description: "多维对比供应商报价与技术偏差" },
  { title: "项目套价中心", description: "基于价格依据快速形成项目测算" },
  { title: "价格证据链", description: "价格来源可追溯，审核依据清晰" },
];
...
export default function LoginPage() {
  return (
    <main className="relative min-h-screen overflow-hidden bg-primary-deep text-white">
      ...
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_64%_28%,rgba(0,166,184,0.26),transparent_34%),radial-gradient(circle_at_34%_78%,rgba(11,92,173,0.22),transparent_36%)]" />
      ...
      <div className="relative z-10 grid min-h-screen grid-cols-1 items-center gap-10 px-6 py-10 lg:grid-cols-[1.15fr_0.85fr] lg:px-20">
        <section className="max-w-4xl">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="mt-1 text-sm text-cyan-100/80">AI Price Intelligence · V4.6</p>
            </div>
          </div>
          ...
          <div className="mt-12">
            ...
            <p className="mt-6 max-w-3xl text-[18px] leading-8 text-blue-50/86">
              面向投标组价、供应商询价、成本测算与价格依据管理的工程价格数据平台。
              AI 负责识别、采集与建议，最终商务判断始终进入人工复核流程。
            </p>
          </div>
        ...
        <section className="mx-auto w-full max-w-[520px] rounded-[24px] border border-white/60 bg-white/92 p-8 text-textMain shadow-[0_28px_80px_rgba(6,27,58,0.34)] backdrop-blur-xl lg:p-10">
          <div className="text-center">
            ...
            <p className="mt-4 text-body text-textMuted">静态模拟登录，不连接真实认证服务。</p>
          </div>
          ...
          <form className="mt-8 flex flex-col gap-5">
            <label className="flex h-14 items-center gap-3 rounded-[12px] border border-borderSoft bg-white px-4 text-textMuted">
              ...
              <input
                defaultValue="demo@water-ai.local"
            ...
            <div className="flex items-center justify-between text-caption text-textMuted">
              <label className="inline-flex items-center gap-2">
                <input type="checkbox" className="size-4 rounded border-borderSoft" />
                记住我
              </label>
              <span>忘记密码？</span>
            </div>
            ...
            <Link
              href="/dashboard"
              className="flex h-14 items-center justify-center rounded-[12px] bg-primary text-[16px] font-semibold text-white shadow-[0_16px_32px_rgba(11,92,173,0.26)] transition hover:bg-primary-navy"
            >
              模拟登录
            </Link>
          </form>
          ...
          <div className="mt-6 flex flex-col items-center gap-2 text-center">
            ...
            <p className="text-caption text-textMuted">AI 输出只作为辅助建议，必须进入人工复核。</p>
          </div>
        </section>
      </div>
    </main>
